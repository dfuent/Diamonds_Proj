...
diamonds_data <- mutate(diamonds_data, pt3_price = (price)^(1/3))
...
diamonds_data <- mutate(diamonds_data, log_carat = log(carat), log_t_price = log(price^(1/3)), log_price = log(price))

head(diamonds_data)

attach(diamonds_data)

t_1_mod <- lm(log_price~log_carat)

plot(log_price~log_carat, main = "Cuberoot Price by Log Carat")
